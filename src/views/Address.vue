<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="page-title-normal checkout-title">
              <h2><span>Shopping address</span></h2>
            </div>
            <div class="addr-list-wrap">
                ...
                <ul>
                  <li v-for="(item, index) in addressListFilter"
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="deleteAddressModal(item.addressId)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" @click="setDefault(item)" v-show="!item.isDefault"><i>Set default</i></a>
                    </div>
              ...
              <div class="shipping-addr-more">
                <a class="up-down-btn" href="javascript:;" @click="showAndHiddenMoreAddress" v-bind:class="{'open': limit > 3}">
                  more
                  <i class="i-up-down">
            ...
            <div class="page-title-normal checkout-title">
              <h2><span>Shopping method</span></h2>
            </div>
            <div class="shipping-method-wrap">
              <div class="shipping-method">
                <ul>
                  <li class="check">
                    <div class="name">Standard shopping</div>
                    <div class="price">Free</div>
                    <div class="shipping-tips">
                      <p>Once shopped，Order should arrive in the destination in 1-7 business days</p>
                    </div>
                  </li>
                </ul>
              </div>
      ...
      <Modal v-show="addressModalShow" :modalShow="addressModalShow" v-on:close="closeModal">
        <p slot="message">
          确认要删除我吗？
        </p>
        <div slot="btnGroup">
          <a href="javascript:;" class="btn-login-double" @click="deleteAddress">确认</a>
  ...
  export default {
    ...
    methods: {
      ...
      deleteAddress() {
        ...
        this.$axios.post('/users/deleteUserAddress', {addressId: this.addressId})
          .then((res) => {
      ...
      getUserAddress() {
        this.$axios.get('/users/getUserAddress').then((res) => {
            if (res.data.status === 0) {
              this.addressList = res.data.result.list;
            }
        });
      },
      showAndHiddenMoreAddress() {
        if (this.limit === 3) {
      ...
      setDefault(list) {
        this.$axios.post('/users/editUserAddress', {addressId: list.addressId})
          .then((res) => {
          if (res.data.status === 0) {
            this.getUserAddress();
          }
